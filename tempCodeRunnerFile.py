...
from bien_doi_deu_nhan_biet import  generate_problem_and_solution
from thang_deu import generate_problem_and_solution_2
app = Flask(__name__)
...
# Load the model
model = tf.keras.models.load_model('physics_model.keras', custom_objects=custom_objects)

# Load the label dictionary
with open('physics_label_dict.json', 'r') as f:
...
def chat():
    ...
    if 'message' not in data:
        return jsonify({'error': 'Thiếu thông điệp'}), 400
    ...
    if state["mode"] == "normal":
        ...
        return jsonify({"response": f"Kết quả dự đoán: {predicted_label}. Bạn muốn tạo bài ở mức độ nào, tôi có thể tạo ở các mức sau: nhận biết, thông hiểu, vận dụng."})

    elif state["mode"] == "ask_difficulty_level":
        difficulty_levels = ["nhận biết", "thông hiểu", "vận dụng"]
        if message in difficulty_levels:
            ...
            return jsonify({"response": "Vui lòng chọn một trong các mức độ: nhận biết, thông hiểu, vận dụng."})

    elif state["mode"] == "ask_number_of_problems":
        try:
            num_problems = int(message)
            if state["predicted_label"] == "chuyển động thẳng biến đổi đều" and state["difficulty_level"] == "nhận biết" and isinstance(num_problems, int):
                problems = []
                ...
                state["mode"] = "normal"
                return jsonify({
                    "response": f"Đã tạo {num_problems} bài tập ở mức độ nhận biết.",
                    "problems": problems
                })
            
            if state["predicted_label"] == "chuyển động thẳng đều" and state["difficulty_level"] == "nhận biết" and isinstance(num_problems, int):
                problems = []
                for _ in range(num_problems):
